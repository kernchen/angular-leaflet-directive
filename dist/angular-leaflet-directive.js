...
leafletDirective.directive('leaflet', function ($http, $log, $parse, $rootScope) {
    ...
    return {
        ...
        transclude: true,
        scope: {
            locfilterbounds: '=locfilterbounds',
            center: '=center',
            maxBounds: '=maxbounds',
            bounds: '=bounds',
            marker: '=marker',
        ...
        link: function ($scope, element, attrs /*, ctrl */) {
            ...
            if ($scope.locfilterbounds){
                console.log("locFilterBounds: " + JSON.stringify($scope.locfilterbounds, undefined, 2));
            } else {
                console.log("no locFilterBounds found");
            }
            ...
            */
            ...
            function setupCenter() {
                if (!$scope.center) {
                    ...
                    console.log("scope.center: " + JSON.stringify($scope.center,undefined,2));
...
*/
                locationFilter.on("change", function(e){
                    console.log("new bounds are: " + JSON.stringify(e.bounds, undefined, 2));
                    $scope.locfilterbounds = {
                        southwest_lng: e.bounds.getSouthWest().lng,
                        southwest_lat: e.bounds.getSouthWest().lat,
                        northeast_lng: e.bounds.getNorthEast().lng,
                        northeast_lat: e.bounds.getNorthEast().lat
                    }
                })
                ...
                $scope.leaflet.locationFilter = locationFilter;

                $scope.$watch("locfilterbounds", function(locfilterbounds) {
                    if (!locfilterbounds) {
                        console.log("no location filter created");
                        return;
                    } else {
                        console.log("new bounds are: " + locfilterbounds.southwest_lng);
                    }
                });
            }
